import { BarcodeView as BarcodeScannerBaseView, formatsProperty, pauseProperty, preferFrontCameraProperty, torchOnProperty, } from './barcodeview-common';
...
const shouldReturnEAN13AsUPCA = (barcodeFormat, value, requestedFormats) => barcodeFormat === 'EAN_13' && value.indexOf('0') === 0;
...
const getBarcodeFormat = (nativeFormat) => {
    if (nativeFormat === AVMetadataObjectTypeQRCode)
        ...
        console.log('Unknown format scanned: ' +
            nativeFormat +
...
const getBarcodeTypes = (formatsString) => {
    ...
    if (formatsString) {
        ...
        for (let format of formats) {
            format = format.trim();
            const nFormat = nativeFormat(format);
            if (nFormat) {
                types.push(nFormat);
            }
        }
...
export function generateBarCode({ text, type, width, height, frontColor, backColor, }) {
...
//# sourceMappingURL=barcodeview.ios.js.map